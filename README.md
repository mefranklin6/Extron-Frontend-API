...
Extron has announced that their non-xi processors will not have a newer version of Python and will be stuck on Python 3.5 as they go end-of-life.  With this project, you can use modern Python, Go, Rust, C#, Java, Javascript, NodeRed or basically any modern language as a backend and extend the useful life of your equipment.

Additionally, the process of deploying a system using Control Script Deployment Utility (CSDU) is cumbersome.  It is a manual process that can only deploy to one room at a time and breaks any hope of a CI/CD pipeline, continuous improvement, and makes minor bugfixes painful if you have a shared codebase.  By using this project, you only have to use CSDU once or whenever your GUI file changes.

## Compatible Devices

Anything that can run Extron Control Script.  You'll need someone who holds the Extron Authorized Programmer cert to initially push the files.
...
3. Instantiate your hardware into the existing lists using the Device Aliases from step 1 into the `src/hardware.py` file.
